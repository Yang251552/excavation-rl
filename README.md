...
1. **Part 1** — [`isaac-lab-manipulation`](<TODO: Part 1 GitHub URL>) — production-framework competency (works end-to-end on standard manipulation benchmarks).
...
Long-term sim-to-real transfer to physical excavation hardware remains the ultimate research direction this work *would* serve — but Part 3 of the portfolio addresses that goal more carefully, by stripping the engineering overhead that Part 2 demonstrated was consuming the research budget. See the [portfolio context](#excavation-rl--diagnostic-case-study-in-rl-for-granular-manipulation) at the top of this README for how Part 3 inherits and refines this direction.
...
v31 establishes the diagnostic conclusion of this campaign: the BC mechanism reproduces across fresh seeds (so cross-seed variance is not the blocker), but PPO health collapses during BC pretrain. Specific conditions that would unblock further work — entropy floor + softer KL anchor, real PhysX bucket-mesh collision via Isaac Lab, recalibrated KL gate threshold against BC-anchored PPO literature — are documented in [`docs/retrospective.md`](docs/retrospective.md) §4.

**Bridge to Part 3 (granular pick).** Two of the three "unblock" conditions above (real PhysX collision; recalibrated KL gate from a literature-supported reference) require leaving the Part 2 codebase behind: the custom Warp pipeline cannot do real bucket-mesh collision without re-implementing what Isaac Lab already provides, and validating a KL-gate threshold against a literature reference is most credible on a task family the literature actually covers. Part 3's *granular pick* keeps Part 2's research theme (granular-media manipulation) but moves to Isaac Lab's standard manipulation stack, where the same diagnostic questions can be re-asked on infrastructure that doesn't introduce new bug surfaces. The single most valuable Part 3 question (per the retrospective): with real PhysX bucket-mesh containment, does the policy still need a containment heuristic, or does Part 2's sticky-grab hack become unnecessary? The answer either way produces a finding — confirming Part 2's hack as a real workaround, or falsifying it as scaffolding that hid the actual mechanism.
...
**What this contributes to the portfolio**: this repo is the *diagnostic substrate* of the three-act story. Part 1 (`isaac-lab-manipulation`) established baseline tool competency. Part 2 — this repo — showed the diagnostic depth Part 1 alone couldn't display, by *deliberately* taking on a task harder than the available compute / time budget could finish. Part 3 (granular pick on Isaac Lab) is where the diagnoses recorded here get re-asked on infrastructure that does not introduce new bug surfaces — so what gets measured there is the research question, not the codebase. The retrospective's [§6](docs/retrospective.md) is the explicit bridge.
